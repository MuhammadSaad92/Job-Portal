...
const AllCompanies = ({click}) => {

  const [currentPage, setCurrentPage] = useState(1)
  const [postPerPage, setPostPerPage] = useState(15)
  const lastPostIndex = currentPage * postPerPage
  const firstPostIndex = lastPostIndex - postPerPage
   const currentCampanies = data.slice(firstPostIndex, lastPostIndex)
  return (
    <div>
      

                   <div className=' border shadow bg-slate-100 flex justify-between  rounded-md  px-5 py-5'>
                      <h2 className='font-semibold text-xl'>Bonafied Companies</h2>
                   ...
                   {
                    currentCampanies.map((value,index)=>{
                      return  <CompanyCard/>
                    })
                   }
                   </div>
